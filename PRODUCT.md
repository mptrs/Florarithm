...
- Roadmap: M1 (done) single device; M2 (done) private-repo sync; M3 (done) photos + QR fallback for a dead sticker; M4 (under way) multi-generation family tree (done), a collection-wide reminder for the predatory-mite sachets (done), milestones on each plant and for the collection (done), per-plant pest tracking with repeat treatments (still to come).
